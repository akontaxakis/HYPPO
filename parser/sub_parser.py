...
def compute_pipeline_metrics_training(artifact_graph, pipeline, uid, X_train, y_train, artifacts, mode,cc,
    ...
    for step_name, step_obj in pipeline.steps:
        ...
        if hasattr(step_obj, 'fit'):
            ...
            if ("Ra" in name or "La" in name or "KN" in name or "LG" in name or "Gr" in name or "Ri" in name or "Li" in name):
            ...
            artifact_graph.add_node(hs_current+"_fit", type="fitted_operator", size=asizeof.asizeof(fitted_operator),cc=cc,frequency=1)
        ...
        if hasattr(step_obj, 'transform'):
            ...
            artifact_graph.add_node(hs_current + "_ftranform", type="train", size=X_temp.size * X_temp.itemsize, cc=cc,frequency=1)
...
def compute_pipeline_metrics_evaluation(artifact_graph, pipeline, uid, X_test, y_test, artifacts):
    ...
    for step_name, step_obj in pipeline.steps:
        ...
        if hasattr(step_obj, 'transform'):
            ...
            artifact_graph.add_node(hs_current + "_tetranform", type="test", size=X_temp.size * X_temp.itemsize,cc = cc + step_time,frequency=1)
        ...
        if hasattr(step_obj, 'predict'):
            ...
            artifact_graph.add_node(hs_current + "_predict", type="test", size=predictions.size * predictions.itemsize,cc=cc + step_time, frequency=1)
...
def compute_pipeline_metrics_training_helix(artifact_graph, pipeline, uid, X_train, y_train, artifacts, mode,cc,budget,
                                      scores_dir='metrics'):
...
def compute_pipeline_metrics_evaluation_helix(artifact_graph, pipeline, uid, X_test, y_test, artifacts,materialized_artifacts,budget):
    loading_speed = 566255240
    X_temp = X_test.copy()
    y_temp = y_test.copy()
    hs_previous = "X_test__"
